...
\hl{We believe that insights gleaned from this experiment will both contribute to generalized knowledge about how to combat the spread of online misinformation and lay a path forward for further exploration of mechanisms. Firstly, our results will help researchers and decision-makers in technology companies and governments to design interventions aimed at combating the spread of COVID-19 misinformation in Kenya and Nigeria - two major producers and consumers of online information in their respective regions of East and West Africa. Secondly, our findings also provide insights into more general knowledge about the way online social media users interact with information and these kinds of interventions frequently used by industry and researchers. Finally, we view this study as a hypothesis-generating exercise and we plan to use the results we obtain with respect to heterogeneity to design future experiments for further explorations of mechanisms, to better understand \textit{why} particular interventions are more successful among certain subgroups.}
...
\hl{While our main goal is to learn the best contextual policy - and see how this policy influences different types of people, we also care about the outcome (reducing the spread of COVID-19 misinformation). Therefore, we also plan to examine how a few select treatments interact with particular covariates of interest. We do this in two ways - first by testing industry-motivated hypotheses and second by testing theoretically-motivated hypotheses, as described below.}

\hl{\textbf{Industry-motivated hypotheses:}}
We will explore whether the following treatments and covariates have significant interaction terms. We select these treatments because these are currently, or in the past were, being implemented by social media companies including Facebook and Twitter. These covariates were also selected as those in which social media companies collect or have access to and could more easily target on the basis of. For our covariates of interest we will divide these into two groups for any binary variables (i.e. gender) and split on the median value for continuous variables to test two subgroups (i.e. age $\geq$ median and age $<$ median). Given that testing these treatments x covariates will result in 5 (treatments) x 6 (covariate subgroup) tests, we will adjust for multiple hypothesis testing using bonferroni corrections.\todo{@Molly - pls adjust to your preferred method of mult. hypothesis adjustment!} 
...
We hypothesize that the three headline-level treatments listed above will perform better among more educated respondents and among women, compared to the less educated and men. We expect that the two respondent-level treatments will reduce sharing of misinformation more among less-educated respondents than those with more education. \textcolor{red}{To evaluate these hypotheses we will marginalize over all other treatments.}\footnote{After collecting pilot data we will submit an updated preanalysis plan in which we will specify and formalize exactly how we plan to test these hypotheses.}


\hl{\textbf{Theory-motivated hypotheses:}}


\hl{\textbf{Best respondent and headline-level treatments:}} In addition to the above heterogeneous treatment hypotheses. We also plan to test heterogeneity with respect to the best performing respondent-level and headline-level treatments, marginalizing over a balanced distribution of the other factors - again taking the median as the splitting point of continuous covariates: 

Specifically, we will test:
\begin{enumerate}
\item How locus of control and age interact with the best respondent-level treatment. 
\item How CRT and education interact with the best headline-level treatment. 
\end{enumerate}



\todo{Do we want to delete rest of this subsection below?}
