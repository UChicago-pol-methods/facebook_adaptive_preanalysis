...
Drawing on the literature on experimental interventions to combat misinformation, we include several treatments designed to reduce the spread of misinformation online, which are targeted both at the respondent level and headline level. This list of treatments also draws on real-world interventions that companies and platforms have instituted to combat misinformation. Treatments are presented in Table~\ref{tab:treatments}. 
...
We code response to the self-reported questions as 1 if the respondent affirms they want to share the post and 0 otherwise. Let $M_i^a$ be the sum of respondent $i$'s pre-test responses to the \textit{misinformation} stimuli and let $T_i^a$ be the sum of respondent $i$'s pre-test responses to the \textit{true} informational stimuli. $M_i^b$ and $T_i^b$ are the respective post-treatment responses. Then $M_i^a, T_i^a, M_i^b, T_i^b \in {0,1,2, 3, 4}$. 
...
\todo{Note changes in hypotheses. Targeting best fixed policy seems to be causing a lot of coding problems--and is a more relevant problem for Toronto project anyhow. So I removed it. It also seems that we are going to have a hard time getting around secondary hypotheses about heterogeneity, so proposing one here. Anything you want to formally test around party?} % LR: only thing I think of to formally test would be something like: the best contextual policy will not achieve higher value than the best fixed policy wrt partisanship  -- but can we look at that with all other covars in the mix? (Would be better if there was some canonical exp. that showed democrats/republicans differentially influenced by intervention - but i dont think that exists...)
We test a secondary hypothesis regarding treatment effect heterogeneity. The literature on how age interacts with response to misinformation is not settled; \cite{wittenberg2020misinformation} note the attention given to age as an important moderator, and the finding that age correlates with propensity to share misinformation \citep[e.g.,]{guess2019less}, but echo the finding by \cite{amazeen2019reinforcing} that older respondents may be more likely to share fact-checks. Consequently, we test a two-sided hypothesis:

  \begin{hypothesis}
Age will moderate treatment effect of the factcheck headline treatment relative to the control, as compared to respondents of below median age. 
  \label{eq:optmax}
  \begin{align*}
...
\end{align*}
\end{hypothesis}
We note that because we do \textit{not} optimize for power for this hypothesis in our design, if the fact-check headline does not perform particularly well, it may be assigned to very few respondents. In that case, we will not be well powered to test this hypothesis, and failure to reject the null may largely speak to a design under-powered for this test. \todo{Rather than partisanship maybe add something based on religiosity - e.g. religiosity (belief + practice) will moderate treatment effect of real info treatment?}
...
%    \end{equation} 
  \item Fit an optimal depth\textcolor{black}{-two} policy tree:
    ...
    \end{align*}
  Store the policy. 
  \item Collect data for the batch: For every new respondent, collect data on their contexts, and assign treatment consistent with $\hat{\pi}_{x}$ + $\epsilon$ uniform random assignment. 
\end{enumerate}
\end{enumerate}
...
To test hypotheses regarding specific heterogeneous treatment effects, e.g., that the headline ``Fact-check'' treatment is more effective for those with above vs. below median age, we again average across the relevant scores, marginalizing over an equal distribution of levels of the respondent factor, and compare estimates across the two groups. \todo{Note that we say the factors are equal here, because comparing the two groups, the distributions might not be the same.}
...
In all analyses, we include the pre-test response strata for true and false stimuli and indicators for individual stimuli.\todo{Ok to combine these?} 
