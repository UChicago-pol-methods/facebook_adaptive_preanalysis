...
Through the chatbot, we collect demographic and other information on respondents. % We use the below covariates for learning conditional treatment assignment probabilities in the chatbot. 
...
\begin{algorithm}
	...
	\Comment{Initialize weight matrices, covariate matrices, and reward vectors, for each treatment condition separately. }
    	\For{$i = 1, \dots, N$}
		\If{$i \in \mathcal{I}_1$}
			...
			\State $X_{w_i} \leftarrow [X_{w_i}:x_i^\top]$ \Comment{The covariate vector is in the appropriate format for the relevant treatment indicators and interactions.}
			\State $r_{w_i} \leftarrow  [r_{w_i}: y_i]$
		\ElsIf{$i \in \mathcal{I}_b$ for $b = 2, \dots, B-1$}
			\If{$i$ first observation in $\mathcal{I}_b$}
				\For{$w\in \ww$}
					\State $B_w \leftarrow X^\top \Xi_w X + \lambda^{CV} \mathbf{I}$ \Comment{$\lambda^{CV}$ is the penalty factor with the minimum mean cross-validated error using the entire observed history of data.}
