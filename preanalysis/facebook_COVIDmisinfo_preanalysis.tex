...
\todo{Should this be moved to a new 3.3.2 Stimuli section?}
...
\section{Analysis}\label{analysis}\todo{Here, we can use a richer covariate set and e.g., information on stimuli. Specify. }
...
We then simulate data generating processes (DGPs) based on the pilot data, with varying heterogeneity. We create these DGPs by fitting a model to each dataset following (\ref{eq:linear_model_full}),\todo{Determine if our data generating model should be more complex. Discuss with James. }  but instead of learning and applying the cross-validated penalty factor $\lambda^{CV}$, we generate models with varying complexity by over- and under-fitting to the data, imposing different penalty factors. In ridge regression, larger penalties will be associated with more parsimonious models, and less heterogeneity. Smaller penalties will be associated with more complex models, and consequently more heterogeneity. This approach allows us to generate heterogeneity that would plausibly exist in the true underlying populations. 
...
\todo{James to review for notational consistency; refer to our implementation of the bandit agent. }
...
\item The largest penalty factor within one standard deviation of cross validated error from no penalization. \todo{Does this make sense? Maybe plot hetereogeneity ratio and see what it looks like?}
...
\end{figure}
\todo{LR: note I changed the recruitment text to not mention covid so we dont bias sample towards ppl that believe/interested in it or high info-seekers}
